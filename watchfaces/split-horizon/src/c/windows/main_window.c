...
static void tick_handler(struct tm *t, TimeUnits units_changed) {
  if (!data_get_boolean(MESSAGE_KEY_PlayAnimations)) {
    // If animations are disabled, just set the time without animation
    set_time(t);
    return;
  }
...
static void main_window_unload(Window *window) {
  ...
  inverter_layer_compat_destroy(top_shade);
  inverter_layer_compat_destroy(bottom_shade);
  inverter_layer_compat_destroy(q1_shade);
  inverter_layer_compat_destroy(q2_shade);
  inverter_layer_compat_destroy(q3_shade);
  inverter_layer_compat_destroy(q4_shade);

  window_destroy(s_main_window);
}

static void reload_config() {
  if (data_get_boolean(MESSAGE_KEY_ShowDate)) {
    // Show date
  ...
  // Update animation and tick interval
  tick_timer_service_unsubscribe();
  if (data_get_boolean(MESSAGE_KEY_PlayAnimations)) {
    // Enable animations
    tick_timer_service_subscribe(SECOND_UNIT, tick_handler);

    layer_set_hidden(inverter_layer_compat_get_layer(top_shade), false);
    layer_set_hidden(inverter_layer_compat_get_layer(bottom_shade), false);
    layer_set_hidden(inverter_layer_compat_get_layer(q1_shade), false);
    layer_set_hidden(inverter_layer_compat_get_layer(q2_shade), false);
    layer_set_hidden(inverter_layer_compat_get_layer(q3_shade), false);
    layer_set_hidden(inverter_layer_compat_get_layer(q4_shade), false);
  } else {
    // Disable animations
    ...
    layer_set_hidden(inverter_layer_compat_get_layer(q2_shade), true);
    layer_set_hidden(inverter_layer_compat_get_layer(q3_shade), true);
    layer_set_hidden(inverter_layer_compat_get_layer(q4_shade), true);
  }
}
