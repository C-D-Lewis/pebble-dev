...
static InverterLayerCompat *progress_layer, *invert_layer;
...
static void main_window_load(Window *window) {
  ...
  progress_layer = inverter_layer_compat_create(GRect(0, 168 - PROGRESS_BAR_HEIGHT, 0, PROGRESS_BAR_HEIGHT), GColorBlack, GColorWhite);
  layer_add_child(root_layer, inverter_layer_compat_get_layer(progress_layer));

  invert_layer = inverter_layer_compat_create(GRect(0, 0, 0, 0), GColorBlack, GColorWhite);
  layer_add_child(root_layer, inverter_layer_compat_get_layer(invert_layer));
}

static void main_window_unload(Window *window) {
  seven_segment_destroy(segment);
  inverter_layer_compat_destroy(progress_layer);
}
...
static void set_progress_bar_value(int perc) {
  ...
  animate(inverter_layer_compat_get_layer(progress_layer),
    layer_get_frame(inverter_layer_compat_get_layer(progress_layer)),
...
static void update_prefs() {
  ...
  if (is_inverted == true) {
    ...
    layer_set_frame(inverter_layer_compat_get_layer(invert_layer), GRect(0, 0, 144, 168));
  } else {
    layer_set_frame(inverter_layer_compat_get_layer(invert_layer), GRect(0, 0, 0, 0));
  }
}