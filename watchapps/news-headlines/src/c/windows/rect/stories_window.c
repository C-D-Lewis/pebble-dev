...
static void regenerate_fake_paragraph_widths() {
  s_fake_para_widths[0] = (rand() % 30) + 20;
  s_fake_para_widths[1] = (rand() % 30) + 20;
  s_fake_para_widths[2] = (rand() % 30) + 20;
}
...
static void title_update_proc(Layer *layer, GContext *ctx) {
  ...
  graphics_fill_rect(
    ...
    GRect(
      bounds.origin.x + 5,
      y,
      bounds.size.w - s_fake_para_widths[0],
      (s_fake_para_widths[0] / 2)
    ),
    0,
    GCornerNone
  );
  ...
  graphics_fill_rect(
    ...
    GRect(
      bounds.origin.x + 5,
      y,
      bounds.size.w - s_fake_para_widths[1],
      (s_fake_para_widths[1] / 2)
    ),
    0,
    GCornerNone
  );
  ...
  graphics_fill_rect(
    ...
    GRect(
      bounds.origin.x + 5,
      y,
      bounds.size.w - s_fake_para_widths[2],
      // Fill the rest...
      300
    ),
    0,
...
static void up_update_proc(Layer *layer, GContext *ctx) {
  ...
  if (connection_service_peek_pebble_app_connection()) {
    ...
    graphics_draw_bitmap_in_rect(
      ctx,
      s_wrench_bitmap,
      GRect(x_margin, y_margin, bitmap_bounds.size.w, bitmap_bounds.size.h)
    );
  }
}
...
static void window_load(Window *this) {
  ...
  const int indicator_margin = STATUS_BAR_H - 3;

  s_up_indicator_layer = layer_create(
    grect_inset(bounds, GEdgeInsets(0, 0, bounds.size.h - indicator_margin - 5, 0))
  );
  ...
  s_down_indicator_layer = layer_create(
    grect_inset(bounds, GEdgeInsets(bounds.size.h - indicator_margin, 0, 0, 0))
  );
